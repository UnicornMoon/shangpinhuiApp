  ...
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        ...
        <button style="width: 100px; height: 38px; margin-left: 10px" @click="getCode(phone)">获取验证码</button>
        <span class="error-msg">错误提示信息</span>
      </div>
      <div class="content">
        <label>登录密码:</label>
...
export default {
  ...
  methods: {
    ...
    async getCode(phone) {
      try {
        if (phone != "") {
          await this.$store.dispatch("getSendCode", phone);
          //正常应该自己填写 现在是直接填了
    ...
    async userRegister(){
      try{
        //如果成功 路由跳转
        const {phone,code,password,password1} = this;  //必须有分号
