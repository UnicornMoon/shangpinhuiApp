            ...
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr, index) in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd
                  ...
                  v-for="(spuSaleValue, index) in spuSaleAttr.spuSaleAttrValueList"
                  :key="spuSaleValue.id"
                  @click="checkValue(spuSaleAttr.spuSaleAttrValueList,spuSaleValue)"
                >
...
export default {
  ...
  computed: {
    ...
    //给子组件的数据 如果服务器数据没有回来，skuInfo是个空对象，不处理后面会报错
    skuImageList() {
  ...
  methods:{
    //产品的售卖属性值切换高亮
    checkValue(spuSaleAttrValueList,spuSaleValue){
      //遍历全部售卖属性值 先设为0
    ...
    async addShopcar(){
      /*
        当不使用async/await时，异步函数会返回一个Promise对象。这个Promise对象代表了异步操作的最终结果，可以使用.then()和.catch()方法来处理它。
        而当使用async/await时，异步函数会自动将结果包装在一个Promise对象中，并在await处等待该Promise对象的解析。在await后面的代码将暂停执行，
        直到Promise对象被解析为成功状态，并返回解析结果。
        带有async的函数 返回的是promise
      */
     /*
        在try块中，包含可能会引发异常的代码。如果在try块中的代码执行过程中发生了异常，那么程序的执行会立即跳转到catch块，
        并执行catch块中的代码，用以处理异常情况。
     */
      // 1：发请求---通知服务器将产品加入到数据库 
      try{
        //成功
         // 2：服务器存储成功后---进行路由跳转传递参数
         await this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.skuId,skuNum:this.skuNum})
        // 路由跳转 第一种：参数直接传 用$route收 缺点： 对象参数刷新后数据会丢失
         this.$router.push({name:'addCartSuccess',
         query:{
          skuNum:this.skuNum
          }})
        //当有复杂信息需要传递时，可以用会话存储（不持久化，会话结束数据消失）
        //第二种：本地存储|会话存储 传参，一般存储的是JSON字符串 JSON.stringify将对象转化为字符串
