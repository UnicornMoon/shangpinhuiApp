...
export default {
  ...
  computed:{
    ...
    isOne(){
      return this.searchParams.order.indexOf('1')!=-1; //包含1
    },
    isTwo(){
      return this.searchParams.order.indexOf('2')!=-1; //包含2
    },
    isDown(){
      return this.searchParams.order.indexOf('desc')!==-1; //包含降序
    },
    //获取search模块展示一共多少数据
    ...mapState({
  ...
  methods:{
    //向服务器发请求获取search模块数据（根据不同参数进行不同展示）
    getData(){
      this.$store.dispatch("getSearchList",this.searchParams)
    },
    removeCategoryName(){
      //将带给服务器的参数置空后再发请求
      //参数可有可无的：如果属性值为空的字符串，还是会把相应字段带给服务器
      //若将字段设为undefined，当前字段就不会带给服务器 提高性能
  ...
  //数据监听：监听路由组件上的属性的属性值变化 后续变化时
  watch:{
    $route(newValue,oldValue){
      Object.assign(this.searchParams,this.$route.query,this.$route.params)
      this.getData();
      //每次请求完毕，应该把相应的1 2 3 级分类id置空
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
    }
